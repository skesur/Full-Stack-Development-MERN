...
router.get("/:roll", (req,res)=>{
    let cd = students.filter(
        (s) => s.rollno == req.params.roll
    )
    if (cd.length > 0) res.send(cd);
    else res.send("No Such Data Exists!!!");
})

router.get("/div/:div/test/:test",(req,res)=>{
    let cd = students.filter(
        (s) => ((s.div==req.params.div) && (s.test==req.params.test))
    )
    if (cd.length > 0) res.send(cd);
    else res.send("No Such Data Exists!!!");
})
